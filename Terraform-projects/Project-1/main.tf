...
resource "null_resource" "deploy" {
  ...
  provisioner "remote-exec" {
      inline = [
       ...
       "node -v",
       "mkdir /home/ec2-user/pizza-luvrs",
      ]
    }
    ...
    provisioner "file" {
        source      = "rest-api.jar"
        destination = "/home/ec2-user/rest-api.jar"
    }

    ### Copy the sh script into the new EC2 ###
    provisioner "file" {
        source      = "rest-api.sh"
        destination = "/home/ec2-user/rest-api.sh"
    }

    provisioner "file" {
        source      = "/Users/yhu/Documents/pizza-luvrs-main/"
        destination = "/home/ec2-user/pizza-luvrs/"
    }
    ...
    connection {
        type     = "ssh"
        user     = "ec2-user"
        private_key = file("yhu-access-key.pem")
